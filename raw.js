...
function append_random_content() {
  ...
  if (next_area) {
    $('.more:last').html(next_area);
  }
  next_area = new_area.clone(); 
  $('#loading-msg').css('top', $(document).height()).fadeIn();
  next_area.load('./Special:Random #content > *', function() {
...
if ($('body.mediawiki').length) {
  ...
  if ($('#more-msg').length === 0) {
    $('#content')
      .next()
      .append('<p id="more-msg" style="border: 1px solid #a7d7f9; background-color: #f6f6f6; text-align:center; color: #a7d7f9; font-size: 2em; margin-left: 5.5em">&darr; <a href="#" style="color:#252525;" onclick="append_random_content(); return false;" >scroll for more</a> &darr;</p>');
  }
